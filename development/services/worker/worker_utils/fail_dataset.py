        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                if not _data_yaml_path.exists():
                    ...
                    is_ultralytics_pretrained = any(
                        str(_model_path_obj).startswith(prefix)
                        and str(_model_path_obj).endswith(".pt")
                ...
                raise DatasetNotFoundError(
                    f"File system error during initial path check: {e}"
                ) from e
            except PermissionError as e:
                # Catch specific PermissionError during early checks
                raise PermissionsError(
                    f"Permission denied when accessing files during initial path check: {e}"
                ) from e

            logger.info(
                f"Validating dataset: '{_data_yaml_path}' with model: '{_model_path_obj}'."
            )
            ...
            try:
                ...
                model_instance = YOLO(
                    str(_model_path_obj)
                )  # Ultralytics expects string paths for YOLO() and .val()

                # model.val() will attempt to load and validate the dataset.
                metrics = model_instance.val(
                    data=str(_data_yaml_path), **final_val_kwargs
                )
                ...
                logger.error(
                    f"An error occurred during Ultralytics dataset validation. Details: {e}",
                    exc_info=True,
                )  # exc_info=True adds traceback

                ultralytics_error_msg = str(e).lower()

                if (
                    "images not found" in ultralytics_error_msg
                    or "labels not found" in ultralytics_error_msg
                ):
                    ...
                    "no labels found" in ultralytics_error_msg
                    or "labels path is invalid" in ultralytics_error_msg
                ):
                    raise DatasetContentError(
                    ...
                    "missing path" in ultralytics_error_msg
                    or "not found" in ultralytics_error_msg
                ):
                    raise DatasetNotFoundError(
