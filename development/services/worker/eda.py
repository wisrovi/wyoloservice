...
class YOLODataValidator:
    ...
    def analyze_class_distribution(self):
        ...
        sorted_class_names = sorted(
            class_counts.keys(),
            key=lambda x: class_names.index(x) if x in class_names else -1,
        )

        # Graficar la distribución de clases
        plt.figure(figsize=(12, 12))
        sns.barplot(
            x=sorted_class_names, y=[class_counts[name] for name in sorted_class_names]
        )
        plt.xlabel("Class")
        plt.ylabel("Count")
                    ...
                    with open(os.path.join(self.label_folder, label_file), "r") as f:
                        for line in f:
                            ...
                            if num_values >= 5:
                                ...
                                logger.warning(
                                    f"Línea con datos insuficientes en {label_file}: {line}"
                                )
            ...
            for split_folder in os.listdir(dataset_path):
                if split_folder in ["train", "test", "val"]:
                    ...
                    if os.path.isdir(split_path):
                        image_paths.extend(
                            self.get_all_images(split_path, split_folder)
                        )
        else:
            with open(self.data_yaml, "r") as f:
                data = yaml.safe_load(f)
                ...
                if img_hash in image_hashes:
                    duplicates.append(
                        (
                            ...
                            (
                                os.path.basename(image_hashes[img_hash][0]),
    ...
    def generate_example_mosaics(
        self, num_mosaics=3, mosaic_size=(3, 3), target_size=(224, 224)
    ):
        image_files = []
        class_names = []
        class_colors = {}
        ...
        if dataset_type == CLASSIFICATION:
            ...
            for mosaic_idx in range(num_mosaics):
                ...
                for img_file in random.sample(
                    image_files, min(mosaic_size[0] * mosaic_size[1], len(image_files))
                ):
                    img_path = img_file
                    img = cv2.imread(img_path)
                    if img is None:
                ...
                if mosaic_images:
                    ...
                    for row_idx in range(mosaic_size[0]):
                        ...
                        for col_idx in range(mosaic_size[1]):
                            ...
                            mosaic[
                                row_offset : row_offset + img.shape[0],
                                col_offset : col_offset + img.shape[1],
                            ] = img
                            col_offset += img.shape[1]
                        row_offset += (
                            mosaic_images[row_idx].shape[0]
                            if row_idx < len(mosaic_images)
                            else 0
                        )
            ...
            for mosaic_idx in range(num_mosaics):
                        ...
                        for label in labels:
                            if isinstance(label[1], tuple):  # Bounding Box (Detection)
                                ...
                                polygon_points = np.array(
                                    polygon_points, dtype=np.float32
                                )
                ...
                if mosaic_images:
                    height, width, _ = mosaic_images[0].shape
                    mosaic = np.zeros(
                        (height * mosaic_size[0], width * mosaic_size[1], 3),
                        dtype=np.uint8,
                    )
                    ...
                    for image_file in os.listdir(class_path):
                        if image_file.endswith((".jpg", ".png", ".jpeg")):
                            ...
                            with Image.open(image_path) as img:
                                x_centers.append(
                                    0.5
                                )  # El centro de la imagen es siempre 0.5 para clasificación
                                y_centers.append(
                                    0.5
                                )  # El centro de la imagen es siempre 0.5 para clasificación
        else:
    ...
    def create_word_report(
        ...
        try:
            ...
            # Portada
            doc.add_paragraph(
                f"Análisis Exploratorio de Datos (EDA) {self.dataset_type}",
                style="Title",
            ...
            doc.add_paragraph("\n", style="Normal")
            doc.add_paragraph(
                f"Autor: {self.author_name}", style="Normal"
            ).alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
            doc.add_paragraph(
                f"Proyecto: {self.project_name}", style="Normal"
            ).alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
            doc.add_paragraph(
                f"Modelo: {self.dataset_type}", style="Normal"
            ).alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
            doc.add_paragraph(
                f"Descripción: {self.content_description}", style="Normal"
            ).alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
            doc.add_paragraph(
            ...
            doc.add_page_break()
            doc.add_paragraph(
                "Distribución de Áreas de Bounding Boxes:", style="Heading 1"
            )
            ...
            doc.add_page_break()
            doc.add_paragraph(
                "Distribución de Relaciones de Aspecto:", style="Heading 1"
            )
            ...
            doc.add_picture(
                os.path.join(
                    self.report_folder, "bbox_center_position_distribution.png"
                ),
                width=Inches(6),
            )
            ...
            doc.add_paragraph(
                "Resultados de Validación con Ultralytics:", style="Heading 1"
            )
            ...
            if small_images:
                doc.add_paragraph(
                    "Imágenes con dimensiones pequeñas encontradas:", style="Heading 2"
                )
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    ...
                    img2, split2 = dup[1]
                    doc.add_paragraph(
                        f"- '{img1}' en '{split1}' y '{img2}' en '{split2}'"
                    )
            else:
            ...
            if small_images:
                conclusion_lines.append(
                    "Existen imágenes con dimensiones pequeñas que podrían afectar el rendimiento del modelo."
                )
            else:
                conclusion_lines.append(
            ...
            if duplicates:
                conclusion_lines.append(
                    "Se encontraron imágenes duplicadas, lo que requiere una revisión y limpieza de estos casos."
                )
            else:
            ...
            if class_distribution:
                ...
                if balance_ratio > CLASS_IMBALANCE_RATIO:
                    conclusion_lines.append(
                        f"La distribución de clases está desbalanceada. La clase '{max_class}' tiene {max_count} etiquetas, "
                        f"mientras que la clase '{min_class}' tiene {min_count} etiquetas. (Gráfica: Distribución de Clases)"
                    )
                else:
                    conclusion_lines.append(
                        "La distribución de clases es equilibrada. (Gráfica: Distribución de Clases)"
                    )
            ...
            if image_dimensions:
                ...
                if (
                    max(widths) - min(widths) > IMAGE_SIZE_VARIATION_THRESHOLD
                    or max(heights) - min(heights) > IMAGE_SIZE_VARIATION_THRESHOLD
                ):
                    conclusion_lines.append(
                        f"Se observa variabilidad significativa en los tamaños de las imágenes. "
                    ...
                    conclusion_lines.append(
                        "Las dimensiones de las imágenes son similares. (Gráfica: Distribución de Tamaños de Imágenes)"
                    )

            # Bounding box area
            if bbox_areas:
                if (
                    max(bbox_areas) / (min(bbox_areas) + 1e-5)
                    > BBOX_AREA_VARIATION_RATIO
                ):
                    conclusion_lines.append(
                        "Se observa variabilidad en las áreas de los bounding boxes, lo que sugiere la presencia de objetos de diferentes tamaños. "
                        "(Gráfica: Distribución de Áreas de Bounding Boxes)"
                    )
                else:
                    conclusion_lines.append(
                        "Las áreas de los bounding boxes son relativamente uniformes. (Gráfica: Distribución de Áreas de Bounding Boxes)"
                    )
            ...
            if aspect_ratios:
                ...
                if unique_ratios > ASPECT_RATIO_DIVERSITY_THRESHOLD:
                    conclusion_lines.append(
                        "Hay una alta diversidad en las relaciones de aspecto, lo cual puede exigir un mayor nivel de flexibilidad en el modelo. "
                        "(Gráfica: Distribución de Relaciones de Aspecto)"
                    )
                else:
                    conclusion_lines.append(
                        "Las relaciones de aspecto son consistentes, lo que puede facilitar el entrenamiento del modelo. "
                        "(Gráfica: Distribución de Relaciones de Aspecto)"
                    )
            ...
            if bbox_aspect_ratios:
                ...
                if ratio_variance > BBOX_ASPECT_RATIO_VARIANCE:
                    conclusion_lines.append(
                        "Las relaciones de aspecto de los bounding boxes varían significativamente, lo que indica una diversidad en la forma "
                        "de los objetos detectados en las imágenes. (Gráfica: Distribución de Relaciones de Aspecto de Bounding Boxes)"
                    )
                else:
                    conclusion_lines.append(
                        "Las relaciones de aspecto de los bounding boxes son consistentes. "
            ...
            if bbox_center_positions:
                ...
                if (
                    len(set(round(x, 1) for x in xs)) < CENTER_POSITION_BIN_COUNT
                    or len(set(round(y, 1) for y in ys)) < CENTER_POSITION_BIN_COUNT
                ):
                    conclusion_lines.append(
                    ...
                    conclusion_lines.append(
                        "Las posiciones centrales de las etiquetas están distribuidas en las imágenes. "
                        "(Gráfica: Distribución de Posiciones Centrales de Bounding Boxes)"
                    )
            ...
            if bbox_width_height:
                ...
                if len(ws) > 0 and len(hs) > 0:
                    ...
                    if corr < BBOX_WH_CORRELATION_THRESHOLD:
                        conclusion_lines.append(
                            "Existe una baja correlación entre el ancho y el alto de los bounding boxes. "
                            "(Gráfica: Dispersión de Ancho vs. Alto de Bounding Boxes)"
                        )
                    else:
                        conclusion_lines.append(
                            "El ancho y el alto de los bounding boxes están correlacionados. "
                            "(Gráfica: Dispersión de Ancho vs. Alto de Bounding Boxes)"
                        )
            ...
            if class_distribution:
                doc.add_paragraph()
                doc.add_paragraph(
                    "Distribución de etiquetas por clase:", style="Normal"
                )
                for name, count in class_distribution.items():
                    p = doc.add_paragraph(
                        f"{name}: {count} etiquetas", style="List Bullet"
                    )
        ...
        try:
            ...
            def is_classification(split_path):
                ...
                if os.path.isdir(labels_path):
                    txt_files = [
                        f for f in os.listdir(labels_path) if f.endswith(".txt")
                    ]
                    if len(txt_files) > 0:
                        return False
                if not all(
